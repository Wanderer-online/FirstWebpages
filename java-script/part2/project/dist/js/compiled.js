...
function calc() {
  ...
  function calcCalories() {
    ...
    if (sex === "female") {
      calcResult.textContent = ((447.6 + 9.2 * weight + 3.1 * height - 4.3 * age) * activity).toFixed(2);
    } else {
      calcResult.textContent = ((88.36 + 13.4 * weight + 4.8 * height - 5.7 * age) * activity).toFixed(2);
    }
  }
  ...
  getCalcStaticData(".calculating__choose_big div", "calculating__choose-item_active");
...
function forms(formSelector, modalTimerID) {
  ...
  function bindPostData(form) {
    ...
    `; // form.append(statusMessage);
      ...
      form.append(statusMessage); //в формах в html всегда должны быть уникальные name="уникальное_имя"! Иначе FormData не сможет найти input-ы и взять из них данные.
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); //пример
      ...
      (0,_services_post_data__WEBPACK_IMPORTED_MODULE_1__.postData)("http://localhost:3000/requests", json).then(data => {
        console.log(data);
...
function closeModal(modalSelector) {
  ...
  modalWindow.classList.add("hide"); //modalWindow.classList.toggle("show");
...
function showModal() {
  fetch("http://localhost:3000/menu").then(data => data.json()).then(res => console.log(res)).catch(e => console.log(e));
}
...
function slider(_ref) {
  ...
  function SliderNavigation(sliderNumber) {
    ...
    currentSlide.textContent = (0,_timer__WEBPACK_IMPORTED_MODULE_0__.addZeroToNumbers)(sliderNumber); //убираем у всех навигационных элементов  класс активности
  ...
  sliderContainer.append(sliderIndicators); //.dataset.imageNumber = i;
  ...
  for (let i = 1; i <= sliderImages.length; i++) {
    ...
    dots.push(dot); // sliderIndicators.innerHTML+= `
  ...
  SliderNavigation(1); // prevSlide.addEventListener("click", function () {
  //   // -
  //   renderSliderImage(-1);
  // });
  // nextSlide.addEventListener("click", function () {
...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (slider); // export {SliderNavigation};
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function showCollectionContent(Collection) {
  ...
  hideTabContent();
  showTabContent(); //здесь идет перебор элементов содержащихся в родителе для переключателей табов пока не будет найден тот, который вызвал событие click. После того как все табы скрываются, его номер позиции в этом родителе передается как номер включаемого таба
  ...
  tabsParent.addEventListener("click", event => {
    ...
    if (target && target.classList.contains(tabsSelector.slice(1))) {
      //tabsSelector = ".tabheader__item", а нужно "tabheader__item"
      tabs.forEach((item, i) => {
        if (target == item) {
          console.log(item);
          hideTabContent();
...
function getNumbersOnly(str) {
  return parseFloat(str); // str.replace(/\W/g, "");
}
